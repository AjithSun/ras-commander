...
@group(0) @binding(0) var<storage, read> values_a: array<f16>;
@group(0) @binding(1) var<storage, read> values_b: array<f16>;
...
fn eval_metric(values: ptr<storage, array<f16>, read>, pixel_idx: u32) -> f32 {
  ...
  for (var t: u32 = 0u; t < params.timesteps; t = t + 1u) {
    let v = f32((*values)[t * pixel_count + pixel_idx]);
